...
import com.btwl.eduservice.entity.chapter.VideoVo;
import com.btwl.eduservice.mapper.EduChapterMapper;
...
public class EduChapterServiceImpl extends ServiceImpl<EduChapterMapper, EduChapter> implements
  ...
  public boolean deleteChapter(String chapterId) {
    ...
    int count = (int) videoService.count(wrapper);
    //判断
    if (count > 0) {//查询出小节，不进行删除
      throw new BrilliantCustomException(20001, "不能删除");
    } else { //不能查询数据，进行删除
      //删除章节

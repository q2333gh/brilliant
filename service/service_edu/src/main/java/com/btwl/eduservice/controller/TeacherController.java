...
import com.btwl.eduservice.entity.vo.TeacherQuery;
 ...
 */
@Api("讲师管理")
@RestController // 表示接口返回的是RESTful的JSON格式数据
...
public class TeacherController {
  ...
  //1 查询讲师表所有数据
  //REST风格
  @ApiOperation(value = "所有讲师列表") // use for swagger display description
  @GetMapping("findAll")
  ...
  @DeleteMapping("{id}") // 需要发来的http的url有这个{id},如localhost:8081/api/deleteXX/1
  public Ret removeTeacher(
      @ApiParam(name = "id", value = "讲师ID", required = true)
      // 用于swagger提示 id的semantics
  ...
  public Ret pageListTeacher(
    ...
    try {
      ...
      //执行自定义异常
      throw new BrilliantCustomException(20001, "执行了自定义异常处理....");
    }
